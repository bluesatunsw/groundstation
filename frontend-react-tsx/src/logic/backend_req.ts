...
export async function getRadioPasses(
  ...
  long: number,
  alt: number
): Promise<n2yo_get_radio_passes> {
  return await fetch(
    `http://127.0.0.1:${port}/radiopasses?norad_id=${satid}&observer_lat=${lat}&
    ...
    .then((res) => res.json())
    .then((res) => {
      return res as n2yo_get_radio_passes
    })
}
...
export async function getPositions(satid: number, pos: gps_pos, num_positions: number): Promise<n2yo_positions> {
  return await fetch(`http://127.0.0.1:${port}/getpositions?norad_id=${satid}&observer_lat=${pos.latitude}&
    ...
    .then((res) => res.json())
    .then((res) => {
      return res as n2yo_positions
    })
}

// Get what's up from backend
export async function getWhatsUp(pos: gps_pos, search_radius: number, cat_id: number): Promise<n2yo_whats_up> {
